...
export async function recalcularDealScores(prisma: PrismaClient): Promise<void> {
  ...
  if (todos.length === 0) {
    console.log("  Sin listings activos para calcular")
    return
  }

  // Solo m2 en rango [50, 450] entran al cálculo de promedios
  const conM2 = todos
    .filter((l) => l.m2Constructed != null && l.m2Constructed >= M2_MIN && l.m2Constructed <= M2_MAX)
    .map((l) => ({ ...l, pricePerM2: l.price / l.m2Constructed! }))

  console.log(`  ${conM2.length}/${todos.length} listings con m² válido (${M2_MIN}-${M2_MAX} m²)`)
  ...
  for (const [zona, prices] of zonaRaw) {
    ...
    if (clean.length > 0) {
      ...
      zonaAvg.set(zona, avg)
      console.log(`  Zona ${zona}: avg $${Math.round(avg).toLocaleString("es-MX")}/m² (${clean.length} listings limpios de ${prices.length})`)
    }
  }
  ...
  for (const l of todos) {
    ...
    await prisma.listing.update({
      where: { id: l.id },
      data: { pricePerM2, zoneAvgPricePerM2: avg ?? null, dealScore },
    })
    updated++
    if (dealScore != null && dealScore >= 10) oportunidades++
  }

  console.log(`  ✅ ${updated} listings actualizados`)
  console.log(`  🎯 ${oportunidades} posibles oportunidades (>10% abajo del promedio)`)
}
